...
namespace kcp2k.unmanaged
{
    public abstract class KcpPeer
    {
        ...
        protected void Reset(KcpConfig config)
        {
            ...
            kcp = new KCP.Kcp(0, RawSendReliable);

            // set nodelay.
            // note that kcp uses 'nocwnd' internally so we negate the parameter
            kcp.SetNoDelay((int)(config.NoDelay ? 1u : 0u), (int)config.Interval, config.FastResend, !config.CongestionWindow ? 1 : 0);
            ...
            // message afterwards.
            kcp.SetMtu((int)(uint)config.Mtu - METADATA_SIZE_RELIABLE);
        ...
        void HandleDeadLink()
        {
            ...
            if (kcp.State == -1)
            {
                // pass error to user callback. no need to log it manually.
                // GetType() shows Server/ClientConn instead of just Connection.
                OnError(ErrorCode.Timeout, $"{GetType()}: dead_link detected: a message was retransmitted {20} times without ack. Disconnecting.");
                Disconnect();
            }
        }
        ...
        void HandleChoked()
        {
            ...
            int total = (int)(kcp.ReceiveQueueCount + kcp.SendQueueCount +
        ...
        void RawSendReliable(byte[] data, int length)
        {
            ...
            // from 0, with 1 byte
            data[0] = (byte)KcpChannel.Reliable;

            // write handshake cookie to protect against UDP spoofing.
            // from 1, with 4 bytes
            Utils.Encode32U(data, 1, cookie); // allocation free
            ...
            // IO send
            ArraySegment<byte> segment = new ArraySegment<byte>(data, 0, length + 1+4);
            RawSend(segment);
        }
